...
export function roomConnectionRoutes(dependencies: ApiDependencies) {
  ...
  app.post('/:id/connect', async (c) => {
    ...
    const service = new AgentConnections(registry, composio), connection = await service.find(input.app, user.id)
    ...
    const turn = { id: turnId, roomId, botId: bot.id, triggerMessageId: trigger.message.id, actorUserId: user.id, replyMode: 'direct' as const, status: 'waiting_approval' as const, model: bot.model ?? config.defaultModel, modelMessages: [], handoffDepth: 0 }
    ...
    const shared = await publicApproval(db, approval)
    hub.broadcastRoom(roomId, { type: 'approval.updated', approval: shared, ts: now })
    ...
    if (await service.connected(connection, user.id)) {
      await resumeConnections(dependencies, connection, user.id, id)
      return c.json({ approval: { ...shared, status: 'approved' }, connectUrl: `/api/connections/complete?approval=${id}` }, 201)
    }
    return c.json({ approval: shared, connectUrl: path(connection, id) }, 201)
  })
  return app
}
