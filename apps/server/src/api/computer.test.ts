...
let memberId: string
let ownerCookie: string, memberCookie: string
...
function request(method: string, url: string, body?: unknown, authenticated = true, cookie = ownerCookie) {
  return app.request(`/api/computer${url}`, { method, headers: { 'content-type': 'application/json', ...(authenticated ? { cookie } : {}) }, ...(body ? { body: JSON.stringify(body) } : {}) })
}
beforeEach(async () => {
...
it('allows members to take control and enforces holder and owner force rules', async () => {
  ...
  expect((await request('POST', '/lease/take', {}, true, memberCookie)).status).toBe(200)
  ...
  expect((await request('POST', '/lease/heartbeat', {}, true, ownerCookie)).status).toBe(403)
  expect((await request('POST', '/lease/release', { force: true }, true, memberCookie)).status).toBe(403)
  expect((await request('POST', '/lease/release', {}, true, ownerCookie)).status).toBe(403)
  expect((await request('POST', '/lease/heartbeat', {}, true, memberCookie)).status).toBe(200)
...
it('reports storage metadata and restricts manual sync to owners', async () => {
  ...
  expect((await request('POST', '/storage/sync', {}, true, memberCookie)).status).toBe(403)
