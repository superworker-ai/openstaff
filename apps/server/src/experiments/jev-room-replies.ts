...
const timeoutMs = Number(process.env.JEV_REPLY_TIMEOUT_MS || 1_200)
if (!Number.isInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 6_000) throw new Error('JEV_REPLY_TIMEOUT_MS must be from 100 to 6000')
...
const experiment = new ReplyDecisionExperiment(new JevDecisionService(process.env.TYPESAFE_API_KEY, process.env.JEV_MODEL || 'jev-latest'), {
  ...
  for (let repeat = 0; repeat < repeats; repeat++) {
    for (const scenario of ROOM_REPLY_SCENARIOS) {
      ...
      try {
        ...
        await f.db.update(bots).set({ name: 'Candidate', job: scenario.job }).where(eq(bots.id, f.botId))
        await f.db.insert(bots).values({ ...original, id: teammate, slug: 'teammate', name: 'Teammate', job: 'Another room participant' })
