...
export class ManagedCredentialsError extends Error {
  constructor() { super('Computer credentials are managed by your host') }
}
...
export class ComputerCredentials {
  constructor(private readonly db: Database, private readonly secrets: Secrets, private readonly managed = false) {}
  async resolve(provider: ComputerProviderId): Promise<ResolvedCredentials> {
    ...
    if (this.managed) return { values, source: Object.keys(values).length ? 'env' : null }
  ...
  isManaged(provider: ComputerProviderId): boolean { return this.managed && Object.keys(environment[provider]).length > 0 }
  async set(provider: ComputerProviderId, values: Record<string, string>, updatedBy?: string): Promise<void> {
    if (this.managed) throw new ManagedCredentialsError()
  ...
  async clear(provider: ComputerProviderId): Promise<void> { if (this.managed) throw new ManagedCredentialsError(); await this.db.delete(computerCredentials).where(eq(computerCredentials.provider, provider)) }
}
