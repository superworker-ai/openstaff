...
export function browserTools(session: BrowserSession) {
  const safe = async <T>(operation: () => Promise<T>): Promise<T | { error: string }> => {
    try { return await session.run(operation) } catch (error) { return { error: error instanceof Error ? error.message : String(error) } }
  }
  ...
  return {
    browser_navigate: tool({ description: 'Navigate the shared browser page to an HTTP(S) URL.', contextSchema: toolContextSchema, inputSchema: z.object({ url: z.url().refine((value) => /^https?:\/\//.test(value), 'Use HTTP or HTTPS') }), execute: ({ url }) => safe(async () => { await (await session.getPage()).goto(url, { waitUntil: 'domcontentloaded' }); await session.screenshot(true); return session.snapshot() }) }),
