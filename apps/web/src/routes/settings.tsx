...
import { Bot, ChartBar, Clock, Cpu, KeyRound, Monitor, Plug, Puzzle, type LucideIcon } from 'lucide-react'
...
const settingsSectionIds = ['providers', 'models', 'usage', 'plugins', 'connections', 'automations', 'computer', 'bots'] as const
type SettingsSection = typeof settingsSectionIds[number]
type SettingsRenderProps = { workspace: WorkspaceSettings; user: User }
type SettingsEntry = { id: SettingsSection; label: string; icon: LucideIcon; group: 'Workspace' | 'Integrations' | 'Runtime' | 'Team'; ownerOnly?: boolean; render: (props: SettingsRenderProps) => ReactNode }
...
function SettingsPage() {
  ...
  const search = Route.useSearch(), visibleSections = settingsSections.filter((item) => !item.ownerOnly || user.role === 'owner'), requestedSection = search.section ?? (search.connected || search.error ? 'connections' : 'providers'), activeEntry = visibleSections.find((item) => item.id === requestedSection) ?? visibleSections[0]!
