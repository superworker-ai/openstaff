...
function InvitationPage() {
  ...
  return <AuthCard title={`Join ${invitation.workspaceName}`} subtitle={`Invitation for ${invitation.email}`}>{verificationSent ? <p role="status" className="text-sm text-emerald-700">Check your email to verify your address, then return to this invitation.</p> : session.data?.user ? <><p className="mb-4 text-sm text-zinc-600">Signed in as {session.data.user.email}</p><AuthError message={error} /><button disabled={busy || session.data.user.email.toLowerCase() !== invitation.email.toLowerCase()} onClick={() => void acceptExisting()} className={authButtonClass}>Accept invitation</button>{session.data.user.email.toLowerCase() !== invitation.email.toLowerCase() && <p className="mt-3 text-sm text-amber-700">Sign in as {invitation.email} to accept.</p>}</> : <><div className="mb-4 grid grid-cols-2 rounded-xl bg-zinc-100 p-1"><button type="button" onClick={() => setMode('signup')} className={`rounded-lg py-2 ${mode === 'signup' ? 'bg-white font-medium' : 'text-zinc-500'}`}>Create account</button><button type="button" onClick={() => setMode('login')} className={`rounded-lg py-2 ${mode === 'login' ? 'bg-white font-medium' : 'text-zinc-500'}`}>Log in</button></div><form onSubmit={submit} className="space-y-3">{mode === 'signup' && <input required autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Your name" className={authInputClass} />}<input disabled value={invitation.email} aria-label="Email" className={`${authInputClass} bg-zinc-50 text-zinc-500`} /><input required minLength={8} type="password" autoComplete={mode === 'signup' ? 'new-password' : 'current-password'} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Password" className={authInputClass} /><AuthError message={error} /><button disabled={busy} className={authButtonClass}>{busy ? 'Please wait…' : mode === 'signup' ? 'Create account and join' : 'Log in and join'}</button></form><Link to="/login" className="mt-5 block text-sm text-zinc-600 underline underline-offset-2">Return to login</Link></>}</AuthCard>
}
