  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose() }}>
    <DialogContent label="Room settings" className="room-settings flex !max-h-[90vh] max-w-xl flex-col !overflow-hidden">
      ...
      <div className="scrollbar-thin min-h-0 overflow-y-auto px-6 pb-6">
        ...
        <label className="mb-5 block text-xs font-medium text-fg-muted">Section<input value={section} onChange={(event) => setSection(event.target.value)} placeholder="Unassigned" className={input} /></label>
