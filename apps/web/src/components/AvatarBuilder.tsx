...
export function AvatarBuilder({ value, onChange, name }: { value: Avatar; onChange: (value: Avatar) => void; name?: string }) {
  ...
  return <aside className="space-y-5"><div ref={stage} className="bot-builder-stage"><span className="bot-builder-floor" /><span ref={shadow} data-part="shadow" className="bot-builder-shadow" /><span className="bot-builder-anchor"><span ref={body} data-part="body" className="bot-builder-body"><BotAvatar {...avatar} size={56} label={name ?? 'Persona preview'} animate /></span></span></div><div className="flex gap-2"><button type="button" aria-label="Play a move" disabled={reduced} onClick={() => motion.current?.play()} className="flex-1 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm">Play a move</button><button type="button" disabled={reduced} onClick={() => onChange(surpriseAvatar(avatar))} className="flex-1 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm">Surprise me</button></div>{reduced && <p className="text-xs text-zinc-500">Motion preview is off while reduced motion is enabled.</p>}
    <fieldset><legend className="mb-2 text-xs font-medium text-zinc-500">Personality</legend><div className="grid grid-cols-2 gap-2">{BOT_PERSONALITIES.map((personality) => <button type="button" key={personality} aria-label={`Personality: ${PERSONALITY_META[personality].label}`} aria-pressed={avatar.personality === personality} onClick={() => change('personality', personality)} className={`flex items-center gap-2 rounded-xl border p-2 text-left ${avatar.personality === personality ? 'border-black' : 'border-zinc-200'}`}><BotAvatar {...avatar} personality={personality} size={28} /><span><span className="block text-sm font-medium">{PERSONALITY_META[personality].label}</span><span className="mt-1 block text-[11px] leading-4 text-zinc-500">{PERSONALITY_META[personality].description}</span></span></button>)}</div></fieldset>
    ...
    <fieldset><legend className="mb-2 text-xs font-medium text-zinc-500">Color</legend><div className="flex flex-wrap gap-2">{BOT_COLORS.map((color) => <button type="button" key={color} aria-label={`Color: ${color}`} aria-pressed={avatar.color.toLowerCase() === color.toLowerCase()} onClick={() => change('color', color)} className={`rounded-xl border p-1.5 ${avatar.color.toLowerCase() === color.toLowerCase() ? 'border-black' : 'border-zinc-200'}`}><BotAvatar {...avatar} color={color} size={28} /></button>)}<label className={`grid h-[42px] w-[42px] cursor-pointer place-items-center rounded-xl border text-[9px] ${!BOT_COLORS.some((color) => color.toLowerCase() === avatar.color.toLowerCase()) ? 'border-black' : 'border-zinc-200'}`}>Custom<input type="color" aria-label="Custom color" value={avatar.color} onChange={(event) => change('color', event.target.value)} className="sr-only" /></label></div></fieldset>
...
function OptionGroup<T extends string>({ legend, values, selected, label, render, onSelect }: { legend: string; values: readonly T[]; selected: T; label: (value: T) => string; render: (value: T) => React.ReactNode; onSelect: (value: T) => void }) {
  return <fieldset><legend className="mb-2 text-xs font-medium text-zinc-500">{legend}</legend><div className="flex flex-wrap gap-2">{values.map((item) => <button type="button" key={item} aria-label={label(item)} aria-pressed={selected === item} onClick={() => onSelect(item)} className={`rounded-xl border p-1.5 ${selected === item ? 'border-black' : 'border-zinc-200'}`}>{render(item)}</button>)}</div></fieldset>
}
