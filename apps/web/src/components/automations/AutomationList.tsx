...
const dot: Record<InvocationStatus, string> = { running: 'bg-blue-500', completed: 'bg-emerald-500', partial_failed: 'bg-amber-500', failed: 'bg-red-500', skipped: 'bg-zinc-300' }
...
export function AutomationRow({ automation, onChanged, onEdit, onHistory, historyOpen, onWebhookKey }: { automation: AutomationView; onChanged: () => void | Promise<void>; onEdit?: () => void; onHistory?: () => void; historyOpen?: boolean; onWebhookKey?: (value: { url: string; webhookKey: string }) => void }) {
  ...
  return <article className="border-b border-zinc-100 py-3 first:pt-0 last:border-0"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h4 className="truncate text-sm font-medium">{automation.name}</h4><span className={`rounded-full px-2 py-0.5 text-[10px] ${automation.enabled ? automation.consecutiveFailures ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800' : 'bg-zinc-100 text-zinc-600'}`}>{statusLabel(automation)}</span></div><p className="mt-1 text-xs text-zinc-500">{trigger}</p>{automation.nextRunAt && <p className="mt-0.5 text-[11px] text-zinc-400">Next: {new Date(automation.nextRunAt).toLocaleString()}</p>}</div><div className="flex min-w-20 justify-end gap-1" aria-label="Recent invocation statuses">{automation.recent.map((status, index) => <span key={`${status}-${index}`} title={status.replace('_', ' ')} className={`h-2 w-2 rounded-full ${dot[status]}`} />)}</div></div><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs"><button disabled={busy} onClick={run}>Run now</button><button disabled={busy} onClick={toggle}>{automation.enabled ? 'Pause' : 'Resume'}</button>{onEdit && <button disabled={busy} onClick={onEdit}>Edit</button>}<button disabled={busy} className="text-red-600" onClick={remove}>Delete</button>{onHistory && <button disabled={busy} onClick={onHistory}>History</button>}{automation.trigger === 'webhook' && onWebhookKey && <button disabled={busy} onClick={regenerate}>Regenerate key</button>}</div>{error && <p role="alert" className="mt-2 text-xs text-red-600">{error}</p>}{historyOpen && <InvocationHistory automationId={automation.id} />}</article>
}
...
export function AutomationList({ room }: { room: RoomView }) {
  ...
  return <section className="mt-5 border-t border-zinc-200 pt-5"><div className="flex items-center justify-between"><h3 className="text-xs font-medium text-zinc-500">Automations</h3><button type="button" onClick={() => { setWebhook(undefined); setForm({}) }} className="text-xs font-medium">New automation</button></div><p className="mt-3 text-[11px] font-medium uppercase tracking-wide text-zinc-400">Start from a template</p><div className="mt-2 flex flex-wrap gap-1.5">{automationTemplates.map((template) => <button type="button" key={template.id} onClick={() => { setWebhook(undefined); setForm({ template }) }} className="rounded-full border border-zinc-200 px-2.5 py-1 text-xs">{template.title}</button>)}</div>{form && <AutomationForm room={room} automation={form.automation} template={form.template} onSaved={saved} onCancel={() => setForm(null)} />}{webhook && <WebhookKeyPanel url={webhook.url} webhookKey={webhook.webhookKey} onClose={() => setWebhook(undefined)} />}<div className="mt-4">{query.data?.automations.map((automation) => <AutomationRow key={automation.id} automation={automation} onChanged={changed} onEdit={() => { setWebhook(undefined); setForm({ automation }) }} onHistory={() => setHistoryId((current) => current === automation.id ? undefined : automation.id)} historyOpen={historyId === automation.id} onWebhookKey={setWebhook} />)}{query.data?.automations.length === 0 && <p className="text-xs text-zinc-400">No automations in this room yet.</p>}</div>{query.error && <p role="alert" className="mt-2 text-xs text-red-600">{query.error.message}</p>}</section>
}
