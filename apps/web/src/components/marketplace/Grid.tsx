...
export function MarketplaceGrid({ children, loading, count, total, warming, q, more, fetching, loadMore }: {
  ...
  return <>
    <div className="sticky top-0 z-10 mb-4 bg-[#f5f5f3]/95 py-3 text-sm text-zinc-500" aria-live="polite">
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-busy={loading || fetching}>
      {loading ? Array.from({ length: 6 }, (_, index) => <div key={index} data-testid="marketplace-skeleton" className="h-72 animate-pulse rounded-2xl border border-zinc-200 bg-white p-5"><div className="h-10 w-10 rounded-xl bg-zinc-100" /><div className="mt-5 h-5 w-2/3 rounded bg-zinc-100" /><div className="mt-3 h-12 rounded bg-zinc-100" /></div>) : children}
    </div>
    {!loading && !count && <p className="py-12 text-center text-zinc-500">{q ? `No results for “${q}”.` : 'No items available yet.'}</p>}
