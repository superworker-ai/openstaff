...
export function AppCard({ item, configured, disabled, onPlugin, onConfigured }: { item: MarketplaceApp; configured: boolean; disabled: boolean; onPlugin: (name: string, id?: string) => void; onConfigured: () => void }) {
  return <article data-testid="marketplace-card" className="flex flex-col rounded-2xl border border-zinc-200 bg-white p-5">
    {item.logo ? <img src={item.logo} alt="" className="mb-4 h-10 w-10 rounded-xl object-contain" /> : <div className="mb-4 grid h-10 w-10 place-items-center rounded-xl bg-zinc-100 text-lg font-semibold">{item.name[0]}</div>}
    <h2 className="font-semibold">{item.name}</h2>
    <p className="mt-2 line-clamp-2 min-h-10 text-sm text-zinc-500">{item.description}</p>
    <span className={`mt-4 self-start rounded-full px-2 py-1 text-xs ${item.status === 'Connected' ? 'bg-emerald-50 text-emerald-700' : 'bg-zinc-100 text-zinc-600'}`}>{item.status}</span>
...
export function SkillCard({ item, disabled, install }: { item: MarketplaceSkill; disabled: boolean; install: () => void }) {
  return <article data-testid="marketplace-card" className="rounded-2xl border border-zinc-200 bg-white p-5">
    <h2 className="font-semibold">{item.manifest?.displayName ?? item.name}</h2>
    <p className="mt-2 text-sm text-zinc-500">{item.manifest?.description ?? item.description}</p>
