...
function ManualClient({ server, onSave, busy }: { server: PluginServerAuth; onSave: (clientId: string, clientSecret: string) => void; busy: boolean }) {
  ...
  return <form className="mt-5 space-y-4" onSubmit={(event) => { event.preventDefault(); onSave(clientId, clientSecret) }}>
    <p className="text-sm text-fg-muted">Set up a client once. Other apps from this provider will reuse it.</p>
    <ol className="list-inside list-decimal space-y-3 text-sm text-fg-muted">
      <li>Create an OAuth client (Web application) in the <a className="text-fg underline" href={google ? 'https://console.cloud.google.com/apis/credentials' : server.issuer ?? undefined} target="_blank" rel="noreferrer">provider console</a>.</li>
      <li>Enable the API for {server.name} in your project.</li>
      <li>Add exactly this redirect URI:<code className="mt-2 block break-all rounded-md bg-surface-3 p-3 text-xs text-fg">{server.redirectUri}</code><button type="button" className="mt-2 text-xs text-fg underline" onClick={async () => { try { await navigator.clipboard.writeText(server.redirectUri); setCopied(true) } catch { setCopied(false) } }}>{copied ? 'Copied' : 'Copy redirect URI'}</button></li>
    </ol>
    <label className="block text-sm">Client ID<input required autoComplete="off" className={inputClass} value={clientId} onChange={(event) => setClientId(event.target.value)} /></label>
    <label className="block text-sm">Client secret<input type="password" autoComplete="new-password" className={inputClass} value={clientSecret} onChange={(event) => setClientSecret(event.target.value)} /></label>
    <button disabled={busy || !clientId.trim()} className={buttonClass}>Connect {server.name}</button>
  </form>
}
...
export function ConnectDialog({ pluginId, serverName, approvalId, standalone = false, onClose }: { pluginId: string; serverName?: string; approvalId?: string; standalone?: boolean; onClose?: () => void }) {
  ...
  const body = <>
    <div className="flex items-center justify-between"><h1 className="text-xl font-semibold">Connect {server?.name ?? 'app'}</h1>{onClose && <button type="button" aria-label="Close connection dialog" onClick={onClose} className="p-2 text-fg-muted hover:text-fg">×</button>}</div>
    {query.data && query.data.servers.length > 1 && <select aria-label="App server" className={`${inputClass} mt-4`} value={server?.name} onChange={(event) => setSelected(event.target.value)}>{query.data.servers.map((item) => <option key={item.name}>{item.name}</option>)}</select>}
    {query.isLoading && <p className="mt-5 text-sm text-fg-muted">Checking connection…</p>}
    {server && ((server.connected || server.auth === 'none') && !editClient
      ? <div className="mt-6"><p role="status" className="font-medium text-ok">Connected</p><p className="mt-2 text-sm text-fg-muted">Connected, you can return to the chat.</p><button disabled={action.busy} className="mt-4 text-sm underline" onClick={() => connect()}>Reconnect {server.name}</button>{standalone && <a href="/" className={`${buttonClass} mt-5 inline-block`}>Return to chat</a>}</div>
      : server.needsClientCredentials || editClient
