...
export function Automations() {
  ...
  return <Section title="Automations">{query.data?.automations.length === 0 && <p className="text-sm text-fg-muted">Create automations from a room's settings, or ask a bot to schedule recurring work.</p>}<div className="space-y-5">{groups.map(({ room, automations }) => <div key={room.id}><h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-fg-subtle">{room.name ?? room.members.find((member) => member.memberKind === 'bot')?.entity?.name ?? 'Room'}</h3>{automations.map((automation) => <AutomationRow key={automation.id} automation={automation} onChanged={changed} />)}</div>)}</div>{query.error && <p role="alert" className="mt-3 text-sm text-danger">{query.error.message}</p>}</Section>
}
