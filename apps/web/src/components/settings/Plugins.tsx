...
import { buttonClass, dangerButtonClass, ErrorText, Section, useAction } from './common'
...
function PluginRow({ plugin, owner, refresh }: { plugin: InstalledPlugin; owner: boolean; refresh: () => Promise<unknown> }) {
  ...
  return <article className="border-t border-line py-5 first:border-0 first:pt-0"><div className="flex items-start gap-3">{plugin.manifest.logo && <img src={plugin.manifest.logo} alt="" className="h-9 w-9 rounded-lg object-contain" />}<div className="flex-1"><h3 className="font-medium">{plugin.manifest.displayName ?? plugin.name}</h3><p className="mt-1 text-sm text-fg-muted">{plugin.manifest.description}</p><p className="mt-2 text-xs text-fg-subtle">{plugin.skills.length} skills{plugin.hooks ? ' · Hooks are unsupported' : ''}</p></div><label className="flex items-center gap-2 text-xs"><input type="checkbox" disabled={!owner || action.busy} checked={plugin.enabled} onChange={(e) => update({ enabled: e.target.checked })} />Enabled</label></div>
    <PluginServers pluginId={plugin.id} owner={owner} />
    {owner && <><div className="mt-4 space-y-3">{Object.entries(plugin.manifest.variables?.properties ?? {}).map(([name, field]) => <label key={name} className="block text-xs font-medium">{field.title ?? name}{plugin.variablesConfigured[name] && <span className="ml-2 text-ok">Configured</span>}<VariableInput field={field} value={values[name]} configured={Boolean(plugin.variablesConfigured[name])} onChange={(value) => setValues({ ...values, [name]: value })} /><span className="mt-1 block font-normal text-fg-subtle">{field.description}</span></label>)}</div><div className="mt-4 flex items-center gap-4">{Object.keys(plugin.manifest.variables?.properties ?? {}).length > 0 && <button disabled={action.busy || !Object.keys(values).length} className={buttonClass} onClick={saveVariables}>Save variables</button>}<button disabled={action.busy} className={dangerButtonClass} onClick={() => action.run(async () => { await api(`/api/plugins/${plugin.id}`, { method: 'DELETE' }); await refresh() })}>Delete plugin</button></div></>}<ErrorText error={action.error} /></article>
}
